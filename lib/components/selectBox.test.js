...
describe("SelectBox", () => {
  ...
  describe("getOptionClassName", () => {
    it("gives a default class name", () => {
      ...
      const className = wrapper.instance().getOptionClassName(options[0]);
      expect(className).to.equal("option");
    });
    it("gives a selected class name", () => {
      const options = [
        new Option("First", 0),
        new Option("Second", 1),
        new Option("Third", 2)
      ...
      const className = wrapper.instance().getOptionClassName(options[1]);
      expect(className).to.equal("option selected");
    });
  });
});
